...
def main():
    ...
    for search_iter in tqdm(range(int(start_search_iter), 100), desc='search progress'):
        ...
        for layer in range(total_layer_num):
            ...
            action = Agent.select_action([layer, last_R,0.01*last_fid] + last_state)
            ...
            if len(memory) >= 64:
                ...
                for i in range(update_time):
                    ...
                    critic_loss, policy_loss = Agent.update_parameters(memory, min(len(memory),256), updates)
       
                    updates += 1 
                    outinfo['critic_error']=critic_loss
                    outinfo['a_loss']=policy_loss
            last_R = R # next step
            last_fid = fid
            last_state = state
        outinfo['rewards']=R
        critic_loss, policy_loss = Agent.update_parameters(memory, len(memory), updates)
        updates += 1 
        outinfo['critic_error']=critic_loss
        outinfo['a_loss']=policy_loss
