...
def train_qin(args, gen_net: nn.Module, dis_net: nn.Module, g_loss_history, d_loss_history, gen_optimizer
    ...
    for epoch in range(shared_epoch):
        for iter_idx, (imgs, _) in enumerate(train_loader):
            ...
            fake_validity, _ = dis_net(fake_imgs)
...
def get_is(args, gen_net: nn.Module, dis_net: nn.Module, num_img,z_numpy=None, get_is_score=True):
    """
    Get inception score.
    :param args:
    :param gen_net:
    ...
    for i in range(eval_iter):
        ...
        dis_state_list.extend(list(dis_states.to('cpu').numpy()))
    state = list(np.mean(state_list, axis=0).flatten())
    dis_state = list(np.mean(dis_state_list, axis=0).flatten())
    if not get_is_score:
        return state
    # get inception score
    ...
    return mean, fid_score, state, dis_state
