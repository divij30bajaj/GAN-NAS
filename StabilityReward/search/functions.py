...
def get_is(args, gen_net: nn.Module, num_img, d_loss_history, z_numpy=None, get_is_score=True):
    """
    Get inception score.
    :param args:
    :param gen_net:
    ...
    if len(d_loss_history) > 2:
        d_loss_var = statistics.variance(d_loss_history)
        d_loss_mean = statistics.mean(d_loss_history)
        mean += abs((d_loss_var + 0.000000001) / (d_loss_mean + 0.000000001)) * 5
