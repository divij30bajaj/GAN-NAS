...
def main():
    ...
    for search_iter in tqdm(range(int(start_search_iter), 50), desc='search progress'):
        ...
        gen_net.set_stage(-1)
        last_R,last_fid,last_state = get_is(args, gen_net, args.rl_num_eval_img, d_loss_history.get_all(), get_is_score=True)
        for layer in range(total_layer_num):
            ...
            R, fid, state = get_is(args, gen_net, args.rl_num_eval_img, d_loss_history.get_all(), z_numpy=Z_NUMPY)
