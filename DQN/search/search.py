...
def main():
    ...
    for search_iter in tqdm(range(int(start_search_iter), 50), desc='search progress'):
        ...
        for layer in range(total_layer_num):
            ...
            arch = Agent.select_action([layer, last_R,0.01*last_fid] + last_state,Best)
            action = np.random.choice(np.arange(len(arch)), p=arch)
            ...
            search_item_lengths = [2, 3, 3, 2, 2, 2]
            cur_arch = [0] * 6
            for search_item in range(6-1, -1 , -1):
                cur_arch[search_item] = action % search_item_lengths[search_item]
                action = int(action / search_item_lengths[search_item])
            ...
            if search_iter >=0: # warm up
                memory.push([layer,last_R,0.01*last_fid]+last_state, action, R-last_R+0.01*(last_fid-fid), [layer+1,R,0.01*fid] + state, mask)  # Append transition to memory
                     
                       
            if len(memory) >= 64:
                for i in range(update_time):
                    loss = Agent.update_parameters(memory, min(len(memory), 256), updates)
                    updates += 1
                    outinfo['dqn_loss'] = loss
                print("full batch", outinfo)

            last_R = R # next step
            last_fid = fid
            last_state = state
        outinfo['rewards']=R
        loss = Agent.update_parameters(memory, len(memory), updates)
        outinfo['dqn_loss'] = loss
        updates += 1 
