...
def main():
    ...
    gen_net.set_arch(args.gen_arch, cur_stage=2)
    dis_net.set_arch(args.dis_arch, cur_stage=2)
    ...
    dis_avg_param = copy_params(dis_net)
    start_epoch = 0
    best_fid = 1e4
    ...
    for epoch in tqdm(range(int(start_epoch), int(args.max_epoch)), desc='total progress'):
        ...
        train(args, gen_net, dis_net, gen_optimizer, dis_optimizer, gen_avg_param, dis_avg_param, train_loader, epoch, writer_dict,
              lr_schedulers)

        if epoch and epoch % args.val_freq == 0 or epoch == int(args.max_epoch)-1:
            backup_param = copy_params(gen_net)
            backup_dis_param = copy_params(dis_net)
            load_params(gen_net, gen_avg_param)
            load_params(dis_net, dis_avg_param)
            ...
            load_params(dis_net, backup_dis_param)
