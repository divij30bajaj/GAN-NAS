...
from models_search.shared_gen_dis import Discriminator
import numpy as np
...
def main():
    ...
    gen_memory = ReplayMemory(2560000)
    dis_memory = ReplayMemory(2560000)
    updates=0
    outinfo = {'rewards': [],
                'a_loss': [],
                'gen_critic_error': [],
    ...
    WARMUP=True
    update_time=1
    for search_iter in tqdm(range(int(start_search_iter), 50), desc='search progress'):
        ...
        gen_net.set_stage(-1)
        dis_net.set_stage(-1)
        last_R,last_fid,last_state,last_dis_state = get_is(args, gen_net, dis_net, args.rl_num_eval_img, get_is_score=True)
        for layer in range(total_layer_num):
            ...
            action = GenAgent.select_action([layer, last_R,0.01*last_fid] + last_state,Best)
            gen_arch = [action[0][0],action[0][1],action[1][0],action[1][1],action[1][2],action[2][0],action[2][1],action[2][2],action[3][0],action[3][1],action[4][0],action[4][1],action[5][0],action[5][1]]
            # print(arch)
            ...
            last_gen_arch += cur_arch
            gen_cur_arch = cur_arch
            gen_net.set_arch(last_gen_arch, layer) # Set the network, given cur_stage
            # Train network

            action = DisAgent.select_action([layer, last_R, 0.01 * last_fid] + last_dis_state, Best)
            dis_arch = [action[0][0], action[0][1], action[1][0], action[1][1], action[1][2], action[2][0], action[2][1],
                    action[2][2], action[3][0], action[3][1], action[4][0], action[4][1], action[5][0], action[5][1]]
            # print(arch)
            # argmax to get int description of arch
            cur_arch = [np.argmax(k) for k in action]
            # Pad the skip option 0=False (for only layer 1 and layer2, not layer0, see builing_blocks.py for why)
            ...
            # Get the network arch with the new architecture attached.
            last_dis_arch += cur_arch
            dis_net.set_arch(last_dis_arch, layer)
            ...
            R, fid, state, dis_state = get_is(args, gen_net, dis_net, args.rl_num_eval_img, z_numpy=Z_NUMPY)
            # Print exploitation mark, for better readability of the log.
            if Best:
                print("Gen arch:", gen_cur_arch, "Dis arch:", cur_arch, "Exploitation:", Best)
            else:
                print("Gen arch:", gen_cur_arch, "Dis arch:", cur_arch, "Exploring...")
            ...
            if search_iter >=0: # warm up
                gen_memory.push([layer,last_R,0.01*last_fid]+last_state, gen_arch, R-last_R+0.01*(last_fid-fid), [layer+1,R,0.01*fid] + state, mask)  # Append transition to memory
                dis_memory.push([layer,last_R,0.01*last_fid]+last_dis_state, dis_arch, R-last_R+0.01*(last_fid-fid), [layer+1,R,0.01*fid] + dis_state, mask)
                       
            if len(gen_memory) >= 64:
                # Number of updates per step in environment
                for i in range(update_time):
                    # Update parameters of all the networks
                    critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = GenAgent.update_parameters(gen_memory, min(len(gen_memory),256), updates)
                    critic_1_dis_loss, critic_2_dis_loss, policy_dis_loss, ent_dis_loss, alpha_dis = DisAgent.update_parameters(dis_memory,
                                                                                                            min(len(
                                                                                                                dis_memory),
                                                                                                                256),
                                                                                                            updates)
                    ...
                    updates += 1 
                    outinfo['gen_critic_error']=min(critic_1_loss, critic_2_loss)
                    outinfo['dis_critic_error'] = min(critic_1_dis_loss, critic_2_dis_loss)
                    outinfo['entropy']=ent_loss
                    outinfo['a_loss']=policy_loss
            last_R = R # next step
            last_fid = fid
            last_state = state
            last_dis_state = dis_state
        outinfo['rewards']=R
        critic_1_loss, critic_2_loss, policy_loss, ent_loss, alpha = GenAgent.update_parameters(gen_memory, len(gen_memory), updates)
        critic_1_dis_loss, critic_2_dis_loss, policy_dis_loss, ent_dis_loss, alpha_dis = GenAgent.update_parameters(gen_memory,
                                                                                                len(gen_memory),
                                                                                                updates)
        updates += 1 
        outinfo['gen_critic_error']=min(critic_1_loss, critic_2_loss)
        ...
        gen_net, dis_net, gen_optimizer, dis_optimizer = create_shared_gan(args, weights_init)
        print(outinfo,len(gen_memory),len(dis_memory))
        GenAgent.save_model("test_gen")
        DisAgent.save_model("test_dis")
        WARMUP=False
